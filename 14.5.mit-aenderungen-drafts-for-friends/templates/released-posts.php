...
<table class="wp-list-table widefat">
	...
	if ( $query->have_posts() ) {
		while ( $query->have_posts() ) {
			$query->the_post();
			$key  = get_post_field( 'dff_key' );
			$link = get_post_permalink();
			$link = add_query_arg( array( 'dff_key' => $key ), $link );
			?>
			<tr>
				...
				<td>
					<?php
					printf(
						'<a href="#" data-key="%s" class="button button-red dff-delete">%s</a>',
						esc_attr( $key ),
						esc_html__( 'Delete', 'drafts-for-friends' )
					);
					?>
				</td>
			</tr>
