...
add_filter( 'plugins_api', 'mm_plugins_api', 10, 3 );

function mm_plugins_api( $original = false, $action, $args ) {
	// $original ist standardmäßig immer false

	// führe diese Funktion nur aus, wenn Plugin Informationen abgerufen werden
	if ( 'plugin_information' != $action ) {
		return $original;
	}

	// Nur wenn der Slug existiert können Daten zurückgegeben werden
	if ( ! isset( $args->slug ) ) {
		return $original;
	}

	// Die durch die Funktion mm_update_plugins() abgerufenen Inhalte werden geholt
	$plugins = get_site_transient( 'update_plugins' );

	// Suche nach dem Plugin-Basename (hier: 'abc-plugin/abc-plugin.php')
	$plugin_basename = array_search( $args->slug, wp_list_pluck( $plugins->response, 'slug' ) );

	if ( false === $plugin_basename ) {
		return $original;
	}

	if ( ! isset( $plugins->response[ $plugin_basename ] ) ) {
		return $original;
	}

	// prüfen, ob es sich um ein externes Plugin handelt
	if ( ! isset( $plugins->response[ $plugin_basename ]->external ) ) {
		return $original;
	}

	// prüfe, ob es sich hier um eines unserer Plugins handelt
	if ( ! isset( $plugins->response[ $plugin_basename ]->mm ) ) {
		return $original;
	}

	return (object) $plugins->response[ $plugin_basename ];
}
