...
function wwi_fetch_weather( $place ) {
	...
	if ( ! function_exists( 'simplexml_load_string' ) ) {
		return new WP_Error(
			'wwi_fetch_temp',
			__( 'This widget-plugin needs the SimpleXML extension from PHP.', 'wi-weather' )
		);
	}
	...
	if ( 2 != substr( $response_code, 0, 1 ) ) {
		return new WP_Error(
			'wwi_fetch_temp',
			sprintf(
				__( 'Could not fetch temperature. Response code was: %s', 'wi-weather' ),
	...
	if ( false === $xml ) {
		return new WP_Error(
			'wwi_fetch_temp',
			__( 'Could not fetch XML data.', 'wi-weather' )
		);
	}
	...
	if ( ! isset( $xml->forecast ) || ! isset( $xml->forecast->tabular ) ) {
		return new WP_Error(
			'wwi_fetch_temp',
			__( 'No forecast data found.', 'wi-weather' )
		);
	}
...
function wwi_plugin_activation() {
	...
	wp_schedule_event( time(), 'hourly', 'wwi_update_weather' );
}
	...
	public function widget( $args, $instance ) {
		...
		if ( is_wp_error( $weather_data ) ) {
			...
			if ( count( $weather_data ) <= 0 ) {
				printf( '<p class="wi-weather-error">%s</p>', __( 'No fresh weather data found.', 'wi-weather' ) );
			} else {
