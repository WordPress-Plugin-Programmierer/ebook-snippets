...
class Link_Table extends \WP_List_Table {
	...
	public function get_columns() {
		...
		return array(
			...
			'count' => __( 'Count', 'track-external-links' ),
		);
	}
	...
	public function process_bulk_action() {
		...
		if ( 'new' === $this->current_action() ) {
			...
			call_user_func( function () {
				...
				if ( $replaced > 0 ) {
					add_settings_error( 'tel', 'update', __( 'Link updated/inserted!', 'track-external-links' ), 'updated' );
				} else {
					add_settings_error( 'tel', 'update', __( 'Link not updated/inserted. Maybe error or nothing has changed or error occurred.', 'track-external-links' ), 'error' );
				}
	...
	/**
	 * Returns an HTML row template for JavaScript usage.
	 *
	 * @since 0.3.0
	 *
	 * @return string
	 */
	public function get_row_template() {
